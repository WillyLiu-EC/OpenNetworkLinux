...
/*
 * A hwmon driver for the as1817_64o_thermal
 *
...
static int as1817_64o_thermal_remove(struct platform_device *pdev);
...
struct as1817_64o_thermal_data *data = NULL;
...
static struct attribute *as1817_64o_thermal_attributes[] = {
...
static const struct attribute_group as1817_64o_thermal_group = {
    .attrs = as1817_64o_thermal_attributes,
};
             ...
             char *buf)
{
    ...
    if (attr->index >= TEMP1_MAX && attr->index <= TEMP14_MAX) {
...
static int as1817_64o_thermal_probe(struct platform_device *pdev)
{
    ...
    status = sysfs_create_group(&pdev->dev.kobj, &as1817_64o_thermal_group);
    if (status)
        goto exit;
...
static int as1817_64o_thermal_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as1817_64o_thermal_group);
