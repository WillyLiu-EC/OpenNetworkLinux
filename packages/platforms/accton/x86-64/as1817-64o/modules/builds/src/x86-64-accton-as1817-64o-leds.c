...
#define LED_COUNT 5
#define LED_UPDATE_INTERVAL 5  /* Cache update interval in seconds */
...
static int as1817_64o_led_remove(struct platform_device *pdev);
...
struct as1817_64o_led_data {
	...
	unsigned char ipmi_resp[LED_COUNT]; /* 0:Loc 1:Diag 2:Alarm 3:Fan 4:Psu */
	struct ipmi_data ipmi;
};


static struct as1817_64o_led_data *data;
...
enum as1817_64o_led_sysfs_attrs {
	LED_LOC,
	LED_DIAG,
	LED_ALARM,
	LED_FAN,
...
static struct attribute *as1817_64o_led_attributes[] = {
...
static const struct attribute_group as1817_64o_led_group = {
	.attrs = as1817_64o_led_attributes,
};

/**
 * as1817_64o_led_update_device - Update LED status from BMC via IPMI
 *
 * This function queries the BMC for current LED states. Data is cached
 * for LED_UPDATE_INTERVAL seconds to reduce IPMI traffic.
 *
 * Context: Must be called with data->update_lock held.
 *
 * Return: Pointer to the driver data structure.
 */
static struct as1817_64o_led_data *as1817_64o_led_update_device(void)
{
	int status = 0;

	if (time_before(jiffies, data->last_updated + HZ * LED_UPDATE_INTERVAL) && data->valid) {
		return data;
	}
...
/**
 * show_led - sysfs read handler for LED attributes
 * @dev: Pointer to the device structure
 * @da: Pointer to the device attribute
 * @buf: Buffer to store the result string
 *
 * This function reads LED status and returns the current mode.
 * PSU, FAN, and DIAG LEDs are hardware-controlled and always report AUTO.
 *
...
/**
 * set_led - sysfs write handler for LED attributes
 * @dev: Pointer to the device structure
 * @da: Pointer to the device attribute
 * @buf: Buffer containing the value to set
 * @count: Number of bytes in @buf
 *
 * This function sets the LED mode based on user input.
 *
 * Return: Number of bytes consumed, or negative errno on failure.
 */
static ssize_t set_led(struct device *dev, struct device_attribute *da,
			const char *buf, size_t count)
{
	...
	data->ipmi_resp[0] = attr->index + 1; /* LED index is 1 based in IPMI */
...
static int as1817_64o_led_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as1817_64o_led_group);
	if (status)
		goto exit;
...
MODULE_DESCRIPTION("AS1817-64O LED Driver");
MODULE_LICENSE("GPL");
