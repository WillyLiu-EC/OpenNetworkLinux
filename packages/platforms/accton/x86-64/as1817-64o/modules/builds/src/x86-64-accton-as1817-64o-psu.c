...
/*
 * A hwmon driver for the as1817_64o_psu
 *
...
static int as1817_64o_psu_remove(struct platform_device *pdev);
...
/* Indices for ipmi_psu_resp_data.status[] array */
enum psu_data_index {
    PSU_PRESENT = 0,
    PSU_TEMP_FAULT,
    PSU_POWER_GOOD_CPLD,
...
/* Indices for ipmi_psu_resp_data.info[] array */
enum psu_info_index {
    PSU_MODEL = 0,
    PSU_SERIAL = 0,
    PSU_TEMP1_MAX0 = 2,
...
struct as1817_64o_psu_data {
    ...
    char valid[NUM_OF_PSU];        /* != 0 if registers are valid, 0: PSU1, 1: PSU2 */
    unsigned long last_updated[NUM_OF_PSU];    /* In jiffies, 0: PSU1, 1: PSU2 */
    struct ipmi_data ipmi;
    struct ipmi_psu_resp_data ipmi_resp[NUM_OF_PSU];    /* 0: PSU1, 1: PSU2 */
    unsigned char ipmi_tx_data[2];
};

static struct as1817_64o_psu_data *data;
...
#define PSU_TEMP_INPUT_ATTR_ID(pid, tid)     PSU##pid##_TEMP_INPUT##tid
#define PSU_TEMP_INPUT_MAX_ATTR_ID(pid, tid) PSU##pid##_TEMP_INPUT##tid##_MAX
#define PSU_TEMP_INPUT_MIN_ATTR_ID(pid, tid) PSU##pid##_TEMP_INPUT##tid##_MIN
...
#define PSU_ATTR(psu_id) \
    ...
    PSU_TEMP_INPUT_ATTR_ID(psu_id, 1), \
    PSU_TEMP_INPUT_ATTR_ID(psu_id, 2), \
    PSU_TEMP_INPUT_ATTR_ID(psu_id, 3), \
    PSU_FAN_INPUT_ATTR_ID(psu_id), \
    PSU_FAN_DIR_ATTR_ID(psu_id), \
    PSU_TEMP_INPUT_MAX_ATTR_ID(psu_id, 1), \
    PSU_TEMP_INPUT_MAX_ATTR_ID(psu_id, 2), \
    PSU_TEMP_INPUT_MAX_ATTR_ID(psu_id, 3), \
    PSU_TEMP_INPUT_MIN_ATTR_ID(psu_id, 1), \
    PSU_TEMP_INPUT_MIN_ATTR_ID(psu_id, 2), \
    PSU_TEMP_INPUT_MIN_ATTR_ID(psu_id, 3), \
...
enum as1817_64o_psu_sysfs_attrs {
    /* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    PSU_ATTR(3),
...
static struct attribute *as1817_64o_psu_attributes[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
    DECLARE_PSU_ATTR(2),
    DECLARE_PSU_ATTR(3),
...
static const struct attribute_group as1817_64o_psu_group = {
    .attrs = as1817_64o_psu_attributes,
};

/**
 * as1817_64o_psu_update_device - Update PSU data from BMC via IPMI
 * @da: Pointer to the device attribute used to infer PSU index
 *
 * This function queries the BMC for PSU status, model, serial number,
 * and fan direction via IPMI commands. Data is cached for
 * PSU_UPDATE_INTERVAL seconds to reduce IPMI traffic.
 *
 * Context: Must be called with data->update_lock held.
 *
 * Return: Pointer to the global driver data structure.
 */
static struct as1817_64o_psu_data *as1817_64o_psu_update_device(struct
                                    device_attribute
                                    *da)
{
    ...
    if (time_before(jiffies, data->last_updated[pid] + HZ * PSU_UPDATE_INTERVAL)
        && data->valid[pid])
        return data;
...
/*
 * VALIDATE_PRESENT_RETURN - Check if PSU is present, unlock and return if not
 * @id: PSU index to check
 *
 * Note: This macro contains hidden control flow (mutex_unlock and return).
 * It must only be used within functions that hold data->update_lock.
 */
...
/**
 * show_psu - sysfs read handler for PSU sensor attributes
 * @dev: Pointer to the device structure
 * @da: Pointer to the device attribute
 * @buf: Buffer to store the result string
 *
 * This function reads PSU sensor values (voltage, current, power, temperature,
 * fan speed, etc.) via IPMI and formats them for userspace consumption.
 *
            ...
            char *buf)
{
    ...
    data = as1817_64o_psu_update_device(da);
    ...
    case PSU4_POUT:
        ...
        value /= 1000;    /* Convert to milliwatt */
        break;
    case PSU1_TEMP_INPUT1 ... PSU1_TEMP_INPUT3:
    case PSU2_TEMP_INPUT1 ... PSU2_TEMP_INPUT3:
    case PSU3_TEMP_INPUT1 ... PSU3_TEMP_INPUT3:
    case PSU4_TEMP_INPUT1 ... PSU4_TEMP_INPUT3:
    {
        u8 data_index;
        VALIDATE_PRESENT_RETURN(pid);
        if (attr->index >= PSU1_TEMP_INPUT1 && attr->index <= PSU1_TEMP_INPUT3)
            data_index = PSU_TEMP1_0 + (attr->index - PSU1_TEMP_INPUT1)*2;
        else if (attr->index >= PSU2_TEMP_INPUT1 && attr->index <= PSU2_TEMP_INPUT3)
            data_index = PSU_TEMP1_0 + (attr->index - PSU2_TEMP_INPUT1)*2;
        else if (attr->index >= PSU3_TEMP_INPUT1 && attr->index <= PSU3_TEMP_INPUT3)
            data_index = PSU_TEMP1_0 + (attr->index - PSU3_TEMP_INPUT1)*2;
        else /* (attr->index >= PSU4_TEMP_INPUT1 && attr->index <= PSU4_TEMP_INPUT3) */
            data_index = PSU_TEMP1_0 + (attr->index - PSU4_TEMP_INPUT1)*2;

        value = ((u32) data->ipmi_resp[pid].status[data_index] |
                (u32) data->ipmi_resp[pid].status[data_index+1] << 8);
        value *= 1000;
        break;
    }
    case PSU1_FAN_INPUT:
...
/**
 * show_psu_info - sysfs read handler for PSU capability/limit attributes
 * @dev: Pointer to the device structure
 * @da: Pointer to the device attribute
 * @buf: Buffer to store the result string
 *
 * This function reads PSU capability and limit information (max/min values
 * for voltage, current, power, temperature) and formats them for userspace.
 *
 * Return: Number of bytes written to @buf, or negative errno on failure.
 */
                 ...
                 char *buf)
{
    ...
    switch (attr->index) {
    case PSU1_TEMP_INPUT1_MAX ... PSU1_TEMP_INPUT3_MAX:
    case PSU2_TEMP_INPUT1_MAX ... PSU2_TEMP_INPUT3_MAX:
    case PSU3_TEMP_INPUT1_MAX ... PSU3_TEMP_INPUT3_MAX:
    case PSU4_TEMP_INPUT1_MAX ... PSU4_TEMP_INPUT3_MAX:
    {
        u8 data_index;
        VALIDATE_PRESENT_RETURN(pid);
        if (attr->index >= PSU1_TEMP_INPUT1_MAX && attr->index <= PSU1_TEMP_INPUT3_MAX)
            data_index = PSU_TEMP1_MAX0 + (attr->index - PSU1_TEMP_INPUT1_MAX) * 4;
        else if (attr->index >= PSU2_TEMP_INPUT1_MAX && attr->index <= PSU2_TEMP_INPUT3_MAX)
            data_index = PSU_TEMP1_MAX0 + (attr->index - PSU2_TEMP_INPUT1_MAX) * 4;
        else if (attr->index >= PSU3_TEMP_INPUT1_MAX && attr->index <= PSU3_TEMP_INPUT3_MAX)
            data_index = PSU_TEMP1_MAX0 + (attr->index - PSU3_TEMP_INPUT1_MAX) * 4;
        else /* (attr->index >= PSU4_TEMP_INPUT1_MAX && attr->index <= PSU4_TEMP_INPUT3_MAX) */
            data_index = PSU_TEMP1_MAX0 + (attr->index - PSU4_TEMP_INPUT1_MAX) * 4;

        value = ((u32) data->ipmi_resp[pid].info[data_index] |
                (u32) data->ipmi_resp[pid].info[data_index+1] << 8);
        break;
    }
    case PSU1_TEMP_INPUT1_MIN ... PSU1_TEMP_INPUT3_MIN:
    case PSU2_TEMP_INPUT1_MIN ... PSU2_TEMP_INPUT3_MIN:
    case PSU3_TEMP_INPUT1_MIN ... PSU3_TEMP_INPUT3_MIN:
    case PSU4_TEMP_INPUT1_MIN ... PSU4_TEMP_INPUT3_MIN:
    {
        u8 data_index;
        VALIDATE_PRESENT_RETURN(pid);
        if (attr->index >= PSU1_TEMP_INPUT1_MIN && attr->index <= PSU1_TEMP_INPUT3_MIN)
            data_index = PSU_TEMP1_MIN0 + (attr->index - PSU1_TEMP_INPUT1_MIN) * 4;
        else if (attr->index >= PSU2_TEMP_INPUT1_MIN && attr->index <= PSU2_TEMP_INPUT3_MIN)
            data_index = PSU_TEMP1_MIN0 + (attr->index - PSU2_TEMP_INPUT1_MIN) * 4;
        else if (attr->index >= PSU3_TEMP_INPUT1_MIN && attr->index <= PSU3_TEMP_INPUT3_MIN)
            data_index = PSU_TEMP1_MIN0 + (attr->index - PSU3_TEMP_INPUT1_MIN) * 4;
        else /* (attr->index >= PSU4_TEMP_INPUT1_MIN && attr->index <= PSU4_TEMP_INPUT3_MIN) */
            data_index = PSU_TEMP1_MIN0 + (attr->index - PSU4_TEMP_INPUT1_MIN) * 4;
...
/**
 * show_string - sysfs read handler for PSU string attributes
 * @dev: Pointer to the device structure
 * @da: Pointer to the device attribute
 * @buf: Buffer to store the result string
 *
 * This function returns PSU identification strings such as model name,
 * serial number, and fan direction.
 *
 * Return: Number of bytes written to @buf, or negative errno on failure.
 */
...
static int as1817_64o_psu_probe(struct platform_device *pdev)
{
    ...
    status = sysfs_create_group(&pdev->dev.kobj, &as1817_64o_psu_group);
    if (status)
        goto exit;
...
static int as1817_64o_psu_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as1817_64o_psu_group);
    return 0;
}
...
MODULE_DESCRIPTION("AS1817-64O PSU Driver");
MODULE_LICENSE("GPL");
