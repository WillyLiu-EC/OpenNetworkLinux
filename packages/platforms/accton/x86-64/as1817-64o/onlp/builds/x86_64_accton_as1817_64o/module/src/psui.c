...
static onlp_psu_info_t pinfo[] = {
    ...
    {
        { ONLP_PSU_ID_CREATE(PSU3_ID), "PSU-3", 0, {0} },
    },
    {
        { ONLP_PSU_ID_CREATE(PSU4_ID), "PSU-4", 0, {0} },
    }
};
...
onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
    ...
    *info = pinfo[pid]; /* Set the onlp_oid_hdr_t */
    ...
    if (ONLP_SUCCESS(ret)) {
        info->hdr.coids[1] = ONLP_THERMAL_ID_CREATE(CHASSIS_THERMAL_COUNT + (pid-1)*3 + 1);
    }

    val = 0;
    ret = onlp_file_read_int(&val, "%s""psu%d_temp2_input", PSU_SYSFS_PATH, pid);
    if (ONLP_SUCCESS(ret)) {
        info->hdr.coids[2] = ONLP_THERMAL_ID_CREATE(CHASSIS_THERMAL_COUNT + (pid-1)*3 + 2);
    }

    val = 0;
    ret = onlp_file_read_int(&val, "%s""psu%d_temp3_input", PSU_SYSFS_PATH, pid);
    if (ONLP_SUCCESS(ret)) {
