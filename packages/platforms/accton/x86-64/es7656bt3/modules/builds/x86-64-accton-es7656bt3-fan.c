/*
 * A hwmon driver for the Accton es7656bt3 fan
 *
...
static struct es7656bt3_fan_data *es7656bt3_fan_update_device(struct device *dev);
...
static struct attribute *es7656bt3_fan_attributes[] = {
...
static int es7656bt3_fan_read_value(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int es7656bt3_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
...
static u32 reg_val_to_duty_cycle(u8 reg_val)
{
    ...
    if( reg_val == 13 || reg_val == 14 || reg_val == 15)
        return 100;
    else
        return (((u32)(reg_val) - 1) * 7 + 18);
}
...
static u8 is_fan_fault(struct es7656bt3_fan_data *data, enum fan_id id)
{
    u8 ret = 1;
                          ...
                          const char *buf, size_t count)
{
    ...
    struct es7656bt3_fan_data *data = i2c_get_clientdata(client);
    int error, value;
    ...
    data = es7656bt3_fan_update_device(dev);
                              ...
                              const char *buf, size_t count)
{
    ...
    es7656bt3_fan_write_value(client, 0x33, 0); /* Disable fan speed watch dog */
    es7656bt3_fan_write_value(client, fan_reg[FAN_DUTY_CYCLE_PERCENTAGE], duty_cycle_to_reg_val(value));
    return count;
}
...
static int _find_lm75_device(struct device *dev, void *data)
{
    ...
    struct es7656bt3_fan_data *prv = data;
...
static const struct attribute_group es7656bt3_fan_group = {
    .attrs = es7656bt3_fan_attributes,
};
...
static struct es7656bt3_fan_data *es7656bt3_fan_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
        ...
        dev_dbg(&client->dev, "Starting es7656bt3_fan update\n");
        data->valid = 0;

        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = es7656bt3_fan_read_value(client, fan_reg[i]);
...
static int es7656bt3_fan_probe(struct i2c_client *client,
                                const struct i2c_device_id *dev_id)
{
    struct es7656bt3_fan_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct es7656bt3_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &es7656bt3_fan_group);
    if (status) {
        goto exit_free;
    }

    data->hwmon_dev = hwmon_device_register_with_info(&client->dev, "es7656bt3_fan",
                                                      NULL, NULL, NULL);
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &es7656bt3_fan_group);
exit_free:
    kfree(data);
exit:
...
static int es7656bt3_fan_remove(struct i2c_client *client)
{
    struct es7656bt3_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id es7656bt3_fan_id[] = {
    { "es7656bt3_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, es7656bt3_fan_id);

static struct i2c_driver es7656bt3_fan_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRVNAME,
    },
    .probe        = es7656bt3_fan_probe,
    .remove       = es7656bt3_fan_remove,
    .id_table     = es7656bt3_fan_id,
    .address_list = normal_i2c,
};

static int __init es7656bt3_fan_init(void)
{
    return i2c_add_driver(&es7656bt3_fan_driver);
}

static void __exit es7656bt3_fan_exit(void)
{
    i2c_del_driver(&es7656bt3_fan_driver);
}

module_init(es7656bt3_fan_init);
module_exit(es7656bt3_fan_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("es7656bt3_fan driver");
MODULE_LICENSE("GPL");
