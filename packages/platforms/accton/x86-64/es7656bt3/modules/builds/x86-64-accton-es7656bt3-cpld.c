 ...
 * This includes the:
 *	 Accton es7656bt3 CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct es7656bt3_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id es7656bt3_cpld_id[] = {
    { "es7656bt3_cpld1", es7656bt3_cpld1 },
    { "es7656bt3_cpld2", es7656bt3_cpld2 },
    { "es7656bt3_cpld3", es7656bt3_cpld3 },
    { }
};
MODULE_DEVICE_TABLE(i2c, es7656bt3_cpld_id);
...
enum es7656bt3_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int es7656bt3_cpld_read_internal(struct i2c_client *client, u8 reg);
static int es7656bt3_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *es7656bt3_cpld3_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group es7656bt3_cpld3_group = {
	.attrs = es7656bt3_cpld3_attributes,
};

static struct attribute *es7656bt3_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
...
static const struct attribute_group es7656bt3_cpld2_group = {
	.attrs = es7656bt3_cpld2_attributes,
};

static struct attribute *es7656bt3_cpld1_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
	&sensor_dev_attr_module_present_all.dev_attr.attr,
...
static const struct attribute_group es7656bt3_cpld1_group = {
	.attrs = es7656bt3_cpld1_attributes,
};
             ...
             char *buf)
{
    ...
    struct es7656bt3_cpld_data *data = i2c_get_clientdata(client);

    if (data->type == es7656bt3_cpld2) {
        u8 byte;
        u8 regs[] = {0x0F, 0x10, 0x11, 0x12};

        mutex_lock(&data->update_lock);
        for (i = 0; i < ARRAY_SIZE(regs); i++) {
            status = es7656bt3_cpld_read_internal(client, regs[i]);
             ...
             char *buf)
{
	...
	case MODULE_RXLOS_57 ... MODULE_RXLOS_58:
		...
		break;
	case MODULE_RESET_49 ... MODULE_RESET_56: /*QSFP*/
		reg  = 0x04 ;
    ...
    mutex_lock(&data->update_lock);
	status = es7656bt3_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
	...
	if (disable) {
        ...
        status = es7656bt3_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	case MODULE_RESET_49 ... MODULE_RESET_56:
		reg  = 0x04 + (attr->index - MODULE_RESET_49)/8;
		mask = 0x1 << ((attr->index - MODULE_RESET_49)%8);
		break;
	default:
		return 0;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = es7656bt3_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void es7656bt3_cpld_add_client(struct i2c_client *client)
{
...
static void es7656bt3_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int es7656bt3_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct es7656bt3_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct es7656bt3_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case es7656bt3_cpld1:
        group = &es7656bt3_cpld1_group;
        break;
    case es7656bt3_cpld2:
        group = &es7656bt3_cpld2_group;
        break;
	case es7656bt3_cpld3:
        group = &es7656bt3_cpld3_group;
        break;
    default:
        break;
    }
    ...
    es7656bt3_cpld_add_client(client);
    return 0;
...
static int es7656bt3_cpld_remove(struct i2c_client *client)
{
    struct es7656bt3_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    es7656bt3_cpld_remove_client(client);
...
int es7656bt3_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = es7656bt3_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(es7656bt3_cpld_read);

int es7656bt3_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = es7656bt3_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(es7656bt3_cpld_write);

static struct i2c_driver es7656bt3_cpld_driver = {
	.driver		= {
		.name	= "es7656bt3_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= es7656bt3_cpld_probe,
	.remove		= es7656bt3_cpld_remove,
	.id_table	= es7656bt3_cpld_id,
};

static int __init es7656bt3_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&es7656bt3_cpld_driver);
}

static void __exit es7656bt3_cpld_exit(void)
{
    i2c_del_driver(&es7656bt3_cpld_driver);
}
...
module_init(es7656bt3_cpld_init);
module_exit(es7656bt3_cpld_exit);
