/*
 * An hwmon driver for accton as5915_16x Power Module
 *
...
#define DRV_NAME        "as5915_16x_psu"
...
enum as5915_16x_psu_sysfs_attributes {
    PSU_INDEX,
    PSU1_PRESENT,
    PSU2_PRESENT,
    PSU_MODEL_NAME,
...
static int as5915_16x_psu_block_read(struct i2c_client *client, u8 command, u8 *data,int data_len);
static int as5915_16x_psu_model_name_get(
    struct device *dev, char *buf);
static int as5915_16x_psu_serial_number_get(
    struct device *dev, enum psu_type type, char *out);
static struct as5915_16x_psu_data *as5915_16x_psu_update_device(struct device *dev);
...
static struct attribute *as5915_16x_psu_attributes[] = {
                          ...
                          char *buf)
{
    ...
    struct as5915_16x_psu_data *data = i2c_get_clientdata(client);
                           ...
                           char *buf)
{
    ...
    struct as5915_16x_psu_data *data = as5915_16x_psu_update_device(dev);
    u8 status = 0;
                                  ...
                                  char *buf)
{
    ...
    i = as5915_16x_psu_model_name_get(dev, buf);
    if ( i < 0) {
        return -ENXIO;
    }

    if (as5915_16x_psu_serial_number_get(dev, i, buf) < 0) {
        return -ENXIO;
    }
                               ...
                               char *buf)
{
    ...
    if (as5915_16x_psu_model_name_get(dev, buf) < 0) {
        return -ENXIO;
    }
...
static const struct attribute_group as5915_16x_psu_group = {
    .attrs = as5915_16x_psu_attributes,
};

static int as5915_16x_psu_probe(struct i2c_client *client,
                                 const struct i2c_device_id *dev_id)
{
    struct as5915_16x_psu_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct as5915_16x_psu_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &as5915_16x_psu_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &as5915_16x_psu_group);
exit_free:
    kfree(data);
exit:
...
static int as5915_16x_psu_remove(struct i2c_client *client)
{
    struct as5915_16x_psu_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id as5915_16x_psu_id[] = {
    { "as5915_16x_psu", as5915_16x_psu },
    {}
};
MODULE_DEVICE_TABLE(i2c, as5915_16x_psu_id);

static struct i2c_driver as5915_16x_psu_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRV_NAME,
    },
    .probe        = as5915_16x_psu_probe,
    .remove       = as5915_16x_psu_remove,
    .id_table     = as5915_16x_psu_id,
    .address_list = normal_i2c,
};

static int as5915_16x_psu_block_read(struct i2c_client *client,
...
static struct as5915_16x_psu_data *as5915_16x_psu_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + UPDATE_PERIOD)
        ...
        dev_dbg(&client->dev, "Starting as5915_16x update\n");
        data->valid = 0;

        /* Read psu status */
        status = as5915_16x_fpga_read(PSU_STATUS_FPGA_I2C_ADDR, PSU_STATUS_I2C_REG_OFFSET);
        if (status < 0) {
            dev_dbg(&client->dev,
        ...
        status = as5915_16x_psu_block_read(client, 0,
...
module_i2c_driver(as5915_16x_psu_driver);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("accton as5915_16x_psu driver");
MODULE_LICENSE("GPL");
