 ...
 * This includes the:
 *   Accton as5915_16x CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct as5915_16x_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id as5915_16x_cpld_id[] = {
    { "as5915_16x_cpld", as5915_16x_cpld },
    { }
};
MODULE_DEVICE_TABLE(i2c, as5915_16x_cpld_id);
...
enum as5915_16x_cpld_sysfs_attributes {
    CPLD_VERSION,
    CPLD_SUB_VERSION,
    ACCESS,
    MODULE_PRESENT_ALL,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int as5915_16x_cpld_read_internal(struct i2c_client *client, u8 reg);
static int as5915_16x_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *as5915_16x_cpld_attributes[] = {
...
static const struct attribute_group as5915_16x_cpld_group = {
    .attrs = as5915_16x_cpld_attributes,
};
             ...
             char *buf)
{
    ...
    struct as5915_16x_cpld_data *data = i2c_get_clientdata(client);

    mutex_lock(&data->update_lock);

    for (i = 0; i < size[data->type]; i++) {
        status = as5915_16x_cpld_read_internal(client, regs[data->type][i]);
        if (status < 0) {
            goto exit;
        }
             ...
             char *buf)
{
    ...
    switch (attr->index) {
    case MODULE_PRESENT_1 ... MODULE_PRESENT_6:
        reg  = 0x10;
        mask = 0x1 << (attr->index - MODULE_PRESENT_1 + 2);
        break;
    case MODULE_PRESENT_7 ... MODULE_PRESENT_12:
        reg  = 0x14;
        mask = 0x1 << (attr->index - MODULE_PRESENT_7);
        break;
    case MODULE_TXDISABLE_1 ... MODULE_TXDISABLE_6:
        reg  = 0x12;
        mask = 0x1 << (attr->index - MODULE_TXDISABLE_1 + 2);
        break;
    case MODULE_TXDISABLE_7 ... MODULE_TXDISABLE_12:
        reg  = 0x16;
        mask = 0x1 << (attr->index - MODULE_TXDISABLE_7);
        break;
    case MODULE_TXFAULT_1 ... MODULE_TXFAULT_6:
        reg  = 0x11;
        mask = 0x1 << (attr->index - MODULE_TXFAULT_1 + 2);
        break;
    case MODULE_TXFAULT_7 ... MODULE_TXFAULT_12:
        reg  = 0x15;
        mask = 0x1 << (attr->index - MODULE_TXFAULT_7);
        break;
    case MODULE_RXLOS_1 ... MODULE_RXLOS_6:
        reg  = 0x13;
        mask = 0x1 << (attr->index - MODULE_RXLOS_1 + 2);
        break;
    case MODULE_RXLOS_7 ... MODULE_RXLOS_12:
        reg  = 0x17;
        mask = 0x1 << (attr->index - MODULE_RXLOS_7);
        break;
    default:
        return 0;
    }
    ...
    if ((attr->index >= MODULE_PRESENT_1) &&
        (attr->index <= MODULE_PRESENT_12)) {
        invert = 1;
    }

    mutex_lock(&data->update_lock);
    status = as5915_16x_cpld_read_internal(client, reg);
            ...
            const char *buf, size_t count)
{
    ...
    status = as5915_16x_cpld_write_internal(client, reg, status);
    if (unlikely(status < 0)) {
        goto exit;
    }
            ...
            const char *buf, size_t count)
{
    ...
    mutex_lock(&data->update_lock);
    status = as5915_16x_cpld_write_internal(client, addr, val);
    if (unlikely(status < 0)) {
        goto exit;
    }
    mutex_unlock(&data->update_lock);
...
static void as5915_16x_cpld_add_client(struct i2c_client *client)
{
...
static void as5915_16x_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int as5915_16x_cpld_probe(struct i2c_client *client,
             const struct i2c_device_id *id)
{
    struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
    struct as5915_16x_cpld_data *data;
    int ret = -ENODEV;
    const struct attribute_group *group = &as5915_16x_cpld_group;

    if (!i2c_check_functionality(adap, I2C_FUNC_SMBUS_BYTE))
        goto exit;

    data = kzalloc(sizeof(struct as5915_16x_cpld_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto exit;
    }
    ...
    as5915_16x_cpld_add_client(client);
    return 0;
...
static int as5915_16x_cpld_remove(struct i2c_client *client)
{
    struct as5915_16x_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = &as5915_16x_cpld_group;

    as5915_16x_cpld_remove_client(client);
...
int as5915_16x_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as5915_16x_cpld_read_internal(cpld_node->client, reg);
            break;
        }
    }
...
EXPORT_SYMBOL(as5915_16x_cpld_read);

int as5915_16x_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as5915_16x_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(as5915_16x_cpld_write);

static struct i2c_driver as5915_16x_cpld_driver = {
    .driver     = {
        .name   = "as5915_16x_cpld",
        .owner  = THIS_MODULE,
    },
    .probe      = as5915_16x_cpld_probe,
    .remove     = as5915_16x_cpld_remove,
    .id_table   = as5915_16x_cpld_id,
};

static int __init as5915_16x_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&as5915_16x_cpld_driver);
}

static void __exit as5915_16x_cpld_exit(void)
{
    i2c_del_driver(&as5915_16x_cpld_driver);
}
...
module_init(as5915_16x_cpld_init);
module_exit(as5915_16x_cpld_exit);
