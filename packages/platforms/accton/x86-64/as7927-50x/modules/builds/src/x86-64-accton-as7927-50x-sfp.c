...
static int as7927_50x_sfp_probe(struct platform_device *pdev);
static int as7927_50x_sfp_remove(struct platform_device *pdev);
static ssize_t show_all(struct device *dev, struct device_attribute *da, char *buf);
static struct as7927_50x_sfp_data *as7927_50x_sfp_update_present(void);
static struct as7927_50x_sfp_data *as7927_50x_sfp_update_txdisable(void);
static struct as7927_50x_sfp_data *as7927_50x_sfp_update_txfault(void);
static struct as7927_50x_sfp_data *as7927_50x_sfp_update_rxlos(void);
static struct as7927_50x_sfp_data *as7927_50x_qsfp_update_present(void);
static struct as7927_50x_sfp_data *as7927_50x_qsfp_update_lpmode(void);
static struct as7927_50x_sfp_data *as7927_50x_qsfp_update_reset(void);
...
struct as7927_50x_sfp_data *data = NULL;

static struct platform_driver as7927_50x_sfp_driver = {
    .probe      = as7927_50x_sfp_probe,
    .remove     = as7927_50x_sfp_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum as7927_50x_sfp_sysfs_attrs {
	SFP_ATTR(1),
    SFP_ATTR(2),
    SFP_ATTR(3),
    SFP_ATTR(4),
...
enum as7927_50x_qsfp_sysfs_attrs {
    QSFP_ATTR(5),
    QSFP_ATTR(6),
    QSFP_ATTR(7),
    QSFP_ATTR(8),
...
static struct attribute *as7927_50x_sfp_attributes[] = {
    /* sfp attributes */
    DECLARE_SFP_ATTR(1),
    DECLARE_SFP_ATTR(2),
    DECLARE_SFP_ATTR(3),
...
static const struct attribute_group as7927_50x_sfp_group = {
    .attrs = as7927_50x_sfp_attributes,
};
...
static ssize_t show_all(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
		case PRESENT_ALL:
        {
            ...
            data = as7927_50x_sfp_update_present();
            if (!data->ipmi_resp.sfp_valid[SFP_PRESENT]) {
                mutex_unlock(&data->update_lock);
                return -EIO;
            }

            data = as7927_50x_qsfp_update_present();
		...
		case RXLOS_ALL:
        {
            ...
            data = as7927_50x_sfp_update_rxlos();
...
static ssize_t show_sfp(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
        ...
        case SFP4_TXDISABLE:
        {
            data = as7927_50x_sfp_update_txdisable();
        ...
        case SFP4_TXFAULT:
        {
            data = as7927_50x_sfp_update_txfault();
...
static ssize_t show_qsfp(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
        ...
        case QSFP40_RESET:
        {
            data = as7927_50x_qsfp_update_reset();
        ...
        case QSFP40_LPMODE:
        {
            data = as7927_50x_qsfp_update_lpmode();
...
static int as7927_50x_sfp_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7927_50x_sfp_group);
    if (status)
        goto exit;
...
static int as7927_50x_sfp_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as7927_50x_sfp_group);

    return 0;
}

static int __init as7927_50x_sfp_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as7927_50x_sfp_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as7927_50x_sfp_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as7927_50x_sfp_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as7927_50x_sfp_exit(void)
{
...
MODULE_DESCRIPTION("AS7927-50X sfp driver");
MODULE_LICENSE("GPL");

module_init(as7927_50x_sfp_init);
module_exit(as7927_50x_sfp_exit);
